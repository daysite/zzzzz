...
// === ANTI-DELETE: GUARDAR Y DETECTAR MENSAJES ===
await antiDelete.saveMessage(sock, m)
await antiDelete.checkDelete(sock, m)

// === CÓDIGO DE PRUEBA PARA VER PROTOCOL MESSAGES ===
if (m.message?.protocolMessage) {
  console.log('[PROTOCOL DETECTADO] Tipo:', m.message.protocolMessage.type)
  console.log('[PROTOCOL] Datos:', JSON.stringify(m.message.protocolMessage, null, 2))
  
  // Si es tipo REVOKE, intentar recuperar manualmente
  if (m.message.protocolMessage.type === 0 || m.message.protocolMessage.type === 'REVOKE') {
    const deletedId = m.message.protocolMessage.key.id
    const chatId = m.message.protocolMessage.key.remoteJid
    console.log(`[MANUAL] Intento de recuperar mensaje: ${deletedId} en ${chatId}`)
    
    if (global.msgStore[chatId] && global.msgStore[chatId][deletedId]) {
      console.log('[MANUAL] ¡Mensaje encontrado en store!')
    } else {
      console.log('[MANUAL] Mensaje NO encontrado en store')
    }
  }
}
