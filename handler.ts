  ...
  try {
    ...
    try {
      ...
      if (msgObj?.stickerMessage) {
        ...
        // Intentar descargar con downloadMediaMessage
        try {
          mediaBuffer = await sock.downloadMediaMessage(deletedMsg)
          if (mediaBuffer) console.log(`[STICKER] Descargado, tamaño: ${mediaBuffer.length} bytes`)
        } catch (e) {
          console.log(`[STICKER] Error download: ${e.message}`)
        }
        
        // Si no funciona, intentar con URL directa
        if (!mediaBuffer && stickerMsg?.url) {
          try {
            ...
            console.log(`[STICKER] Error URL: ${e.message}`)
          }
        }
